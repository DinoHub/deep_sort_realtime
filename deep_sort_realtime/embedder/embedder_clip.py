...
class Clip_Embedder:
    ...
    - model_wts_path (optional, str): Optional specification of path to CLIP model weights. Defaults to None and look
    for weights in `deep_sort_realtime/embedder/weights` or clip will download from internet into their own cache.
    ...
    def predict(self, np_images):
        ...
        all_feats = []
        for this_batch in batch(pil_images, bs=self.max_batch_size):
            curr_batch = torch.stack(this_batch, 0)
            with torch.no_grad():
                feats = self.model.encode_image(curr_batch)
